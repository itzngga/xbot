...
export default class Scrap {
  ...
  ss(link: string): Promise<Buffer | string> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        page
          ...
          .catch(async () => {
            reject('Maaf, url tidak valid');
            await page.close();
          });
      } catch (error) {
        return reject('Maaf, url tidak valid');
      }
    });
  }
  textpro1input(link: string, text: string): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        const browser = this.browser!;
        const page = await browser.newPage();
  ...
  textpro2input(link: string, text1: string, text2: string): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        const browser = this.browser!;
        const page = await browser.newPage();
  ...
  photofunia(link: string, file: string) {
    return new Promise(async (resolve, reject) => {
      try {
        const browser = this.browser!;
        const page = await browser.newPage();
  ...
  instagram(link: string): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        const browser = this.browser!;
        const page = await browser.newPage();
        ...
        page
          ...
          .then(async () => {
            await page.type(
              '#main_form > div.main_input_wrapper > input',
              link
            );
  ...
  webp2mp4(path: string): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        const browser = this.browser!;
        const page = await browser.newPage();
