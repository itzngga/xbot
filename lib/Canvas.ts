    ...
    for (let j = 0; j < width; j++) {
      ...
      const grey = Number.parseInt(
        0.2125 * data.data[dest] +
          ...
          (0.0721 * data.data[dest + 2]).toString(),
        10
      );
...
export function hasAlpha(image: Image) {
  const canvas = createCanvas(image.width, image.height);
  const ctx = canvas.getContext('2d');
  ctx.drawImage(image, 0, 0);
  const data: any = ctx.getImageData(0, 0, canvas.width, canvas.height);
...
export function shortenText(ctx: any, text: string, maxWidth: number) {
  let shorten = false;
...
export function wrapText(ctx: any, text: string, maxWidth: number) {
...
export function centerImage(base: any, data: any) {
