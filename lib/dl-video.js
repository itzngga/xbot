...
const api = fs.readJSONSync('./json/config.json');
function apikey() {
  const ran = Math.floor(Math.random() * 3);
  return api.apikeys.keepsaveit[ran];
}
    ...
    await fetchJson(keepsaveit + '?api_key=' + apikey() + '&url=' + url, {
      ...
      .then(result => {
        ...
        switch (key) {
          ...
          case 212:
            return reject(
              'Access block for you, You have reached maximum 5 limit per minute hits, please stop extra hits.'
            );
            break;
          case 101:
          ...
          case 104:
            return reject(
              'API Key error : You have not set your api_key in parameters.'
            );
            break;
          case 111:
          ...
          case 112:
            return reject(
              'Sorry, Something wrong, or an invalid link. Please try again or check your url.'
            );
            break;
          case 113:
          ...
          case 405:
            return reject(
              "You can't download media in private profile. Looks like the video you want to download is private and it is not accessible from our server."
            );
            break;
        }
      })
