...
import { settingType , stickerType, countType, autoReply, cloud, configType, travellerType, textproType} from '../types/index';
const loginData = new JsonDB('../json/loginData.json', true, false);

export const isHasLoginData = (jid: string): boolean => loginData.exists('/'+jid)
export const login = (jid: string): AnyAuthenticationCredentials => loginData.getObject<AnyAuthenticationCredentials>('/'+jid)
export const addLogin = (jid: string, auth: AnyAuthenticationCredentials) => loginData.push('/'+jid, auth)
...
const isExist2 = (genshin: JsonDB, path: string, json: any) => {
...
export class DB {
    ...
    constructor(jid: string) {
        ...
        isExist(this._db, '/fakeReplyBase64', '../image/fakeimage.jpeg', true, {encoding: 'base64'});
